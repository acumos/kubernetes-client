...
public class ParseJSON {
	...
	public  Blueprint jsonFileToObject(String jsonFile,DataBrokerBean dataBrokerBean)throws  Exception{
        ...
        Map orchestrator = ((Map)jo.get(DockerKubeConstants.ORCHESTRATOR));
        if(orchestrator!=null){
	        ...
	        while (itr1.hasNext()) {
	            ...
	            if(key!=null && key.equalsIgnoreCase(DockerKubeConstants.NAME)){
	            	orchestratorBean.setName(value);
	             }
	            if(key!=null && key.equalsIgnoreCase(DockerKubeConstants.VERSION)){
	...
	/*public static void main(String[] args) {
		try {
			BufferedReader reader = new BufferedReader(new FileReader("deploy/private/setup_k8s.sh"));
			...
			Object obj  = new JSONParser().parse(new StringReader(content));
